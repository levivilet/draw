...
const getPanelClassName = (popupEnabled: boolean): string => {
  return popupEnabled
    ? 'DrawCardDetailPanel DrawCardDetailPanelPopup'
    : 'DrawCardDetailPanel'
}
...
const renderCardAttachmentDropArea = (
  ...
  return [
    {
      childCount: 1,
      className: 'DrawCardAttachmentDropArea',
      type: VirtualDomElements.Div,
    },
    text(
      uploading
        ? DrawStrings.uploadingFiles()
        : DrawStrings.dropFilesToUpload(),
    ),
  ]
}
...
export const renderCardDetailPanel = (
  ...
  if (cardDetailLoading && !selectedCardDetail) {
    const panel = [
      ...
      ...renderListTitle(DrawStrings.cardDetails()),
      text(DrawStrings.loadingCard()),
    ]
  ...
  const content = [
    ...
    ...renderListTitle(DrawStrings.comments()),
